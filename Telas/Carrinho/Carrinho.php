...
if ($has_jogo_genero) {
    $sql = "SELECT j.ID_jogo AS produto_id,
                   j.nome,
                   j.preco,
                   j.desconto,
                   j.Img,
...
while ($row = $result->fetch_assoc()) {
    ...
    $row['quantidade'] = 1;  
    $row['cart_id'] = $row['produto_id'];  
    $row['Img_base64'] = $row['Img'] ? base64_encode($row['Img']) : null;
  ...
  <div class="cart-box">
    <div class="cart-items" id="cart-items">
      <?php if (!empty($cart_items)): ?>
        <?php foreach ($cart_items as $row): ?>
          <div class="cart-item" data-id="<?php echo $row['cart_id']; ?>" data-preco="<?php echo $row['preco_final']; ?>">
            <img src="data:image/jpeg;base64,<?php echo $row['Img_base64']; ?>" 
            ...
            <div class="cart-price">
              <span>R$ <?php echo number_format($row['preco_final'],2,',','.'); ?></span>
              <form method="post" style="display:inline;" class="remove-form">
                  <input type="hidden" name="remove_id" value="<?php echo (int)$row['cart_id']; ?>">
                  <button type="button" class="remove-icon"><i class="mdi mdi-trash-can-outline"></i></button>
              </form>
            </div>
          </div>
        <?php endforeach; ?>
    ...
    <div class="cart-summary">
      ...
      <div class="cart-buttons">
          ...
          <button class="blue-btn-two" onclick="window.location.href='loja.php';">Continuar Comprando</button>
      </div>
    </div>
  </div>
</div>
